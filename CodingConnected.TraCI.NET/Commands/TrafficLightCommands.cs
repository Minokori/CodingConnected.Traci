...
public class TrafficLightCommands(ITcpService tcpService, ICommandHelperService helper) : TraCICommandsBase(tcpService, helper)
    ...
    public List<string> GetIdList()
        {
        var result = _helper.ExecuteGetCommand("ignored", TraCIConstants.CMD_GET_TL_VARIABLE, TraCIConstants.ID_LIST);
    ...
    public int GetIdCount()
        {
        var result = _helper.ExecuteGetCommand("ignored", TraCIConstants.CMD_GET_TL_VARIABLE, TraCIConstants.ID_COUNT);
    ...
    public string GetState(string id)
        {
        var result = _helper.ExecuteGetCommand(id, TraCIConstants.CMD_GET_TL_VARIABLE, TraCIConstants.TL_RED_YELLOW_GREEN_STATE);
    ...
    public double GetPhaseDuration(string id)
        {
        var result = _helper.ExecuteGetCommand(id, TraCIConstants.CMD_GET_TL_VARIABLE, TraCIConstants.TL_PHASE_DURATION);
    ...
    public List<string> GetControlledLanes(string id)
        {
        var result = _helper.ExecuteGetCommand(id, TraCIConstants.CMD_GET_TL_VARIABLE, TraCIConstants.TL_CONTROLLED_LANES);
        return ((TraCIStringList)result.Value).Value;
        }
    ...
    public int GetCurrentPhase(string id)
        {
        var result = _helper.ExecuteGetCommand(id, TraCIConstants.CMD_GET_TL_VARIABLE, TraCIConstants.TL_CURRENT_PHASE);
        return ((TraCIInteger)result.Value).Value;
        }
    ...
    public string GetCurrentProgram(string id)
        {
        var result = _helper.ExecuteGetCommand(id, TraCIConstants.CMD_GET_TL_VARIABLE, TraCIConstants.TL_CURRENT_PROGRAM);
        return ((TraCIString)result.Value).Value;
        }
    ...
    public double GetNextSwitch(string id)
        {
        var result = _helper.ExecuteGetCommand(id, TraCIConstants.CMD_GET_TL_VARIABLE, TraCIConstants.TL_NEXT_SWITCH);
        return ((TraCIDouble)result.Value).Value;
        }
...
#warning is the try catch necessary?
        try
            {
            return TraCIDataConverter.ExtractDataFromResults(response, TraCIConstants.CMD_SET_TL_VARIABLE, TraCIConstants.TL_COMPLETE_PROGRAM_RYG);
            }
        catch
            {
            throw;
