...
/// PositionRandom describes the position of the node in longitudinal direction (ranging from 0 to the road's length).
...
/// </remarks>
public sealed class RoadMapPosition : TraciCompoundObject, ITraciParserable<RoadMapPosition>
    {
    ...
    public static RoadMapPosition Parse(ReadOnlySpan<byte> sourceBytes, out ReadOnlySpan<byte> remainingBytes)
        {
        var roadId = TraciString.Parse(sourceBytes, out sourceBytes);
        var pos = TraciDouble.Parse(sourceBytes, out sourceBytes);
        var laneId = TraciByte.Parse(sourceBytes, out remainingBytes);
