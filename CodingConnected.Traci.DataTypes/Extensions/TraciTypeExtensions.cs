...
public static class TraciTypeExtensions
    ...
    extension(TraciCompoundObject content)
        {
        public List<ControlledLinks> ToControlledLinks()
            {
        ...
        public List<EdgeInformation> ToEdgeInformationList()
            {
        ...
        public List<Foe> ToJunctionFoes() =>
            [.. content.Skip(1).Chunk(9).Select(i => (Foe)i.ToList())];

        public List<Link> ToLinks() => [.. content.Skip(1).Chunk(8).Select(i => new Link(i))];

        public List<StopData> ToStopDataList() =>
            [.. content.Skip(1).Chunk(16).Select(i => new StopData(i))];

        public List<TrafficLightLogic> ToTrafficLightLogics()
            {
        ...
        public List<UpcomingTrafficLights> ToUpcomingTrafficLights() =>
            [.. content.Skip(1).Chunk(4).Select(i => (UpcomingTrafficLights)i.ToList())];

        public List<VehicleInformationPacket> ToVehicleInformationPackets() =>
