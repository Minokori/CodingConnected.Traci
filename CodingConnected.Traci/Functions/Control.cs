...
public class Control(
    ...
    public (int traciApiVersion, string sumoVersion) GetVersion()
        {
        var command = Helper.GenerateCommand(CommandIdentifier.GetVersion);
        var results = SumoHelper.SendMessage(command);
        switch ((results[0] as IStatusResponse).Result)
            {
            case ResultCode.Success:
                {
                var apiVersion = TraciInteger.Parse(results[1].Content, out var remainingBytes);
                var version = TraciString.Parse(remainingBytes, out _);
                if (apiVersion != TraciConstants.TraciVersion)
                    {
                    Console.WriteLine(
                        $"Warning: TraCI API version mismatch."
                            + $" SUMO installed with API version : {apiVersion}."
                            + $" This library is using API version {TraciConstants.TraciVersion}."
                    );
                    }
    ...
    /// if you have any subscription, the responses will be handled by the <see cref="ITraciEventService"/>. Please add your event handler to use the responses.<para/>
    /// </summary>
    ...
    public void SimStep(double targetTime = 0)
        ...
        var command = Helper.GenerateCommand(
            SimulationStep,
    ...
    public void ExecuteMove()
        {
        var command = Helper.GenerateCommand(CommandIdentifier.ExecuteMove);
        _ = SumoHelper.SendMessage(command);
        }
    ...
    public void Close()
        {
        var command = Helper.GenerateCommand(CommandIdentifier.Close);
        _ = SumoHelper.SendMessage(command);
        }
    ...
    public void SetOrder(int index)
        {
        var command = Helper.GenerateCommand(CommandIdentifier.SetOrder, extendParameter: new TraciInteger(index));
        _ = SumoHelper.SendMessage(command);
        }
    }
