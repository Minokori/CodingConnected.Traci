...
public static class TraciResultExtension
    ...
    /// <returns></returns>
    public static IAnswerFromSumo? ExtractData(
        this IEnumerable<TraciResult> results,
        byte commandType,
        byte? variableType = 0
    )
        {
    ...
    /// <param name="results"></param>
    /// <returns></returns>
    public static List<TraciSubscriptionResponse>? ExtractSimStepData(
        this List<TraciResult> results
    )
        {
        ...
        (var statusResponse, var i) = results
            ...
            .FirstOrDefault(x => x.result.Identifier == CommandIdentifier.ControlVariables.SIMSTEP);
        if (statusResponse is null)
            {
            return null;
            }
    ...
    internal static (ITraciType traciValue, IEnumerable<byte> remainBytes) GetValueFromTypeAndArray(
        byte type,
        ...
        switch ((DataType)type)
            ...
            case DataType.COMPOUND:
                ...
                var innerValueNumber = ToInt32(
                    bytes.Take(DataSize.INTEGER_SIZE).Reverse().ToArray()
                );
    ...
    public static List<TraciResult> AsTraCIResults(this List<byte> responseBytes)
        ...
        if (totalLengthOfBytes != responseBytes.Count)
            {
            throw new Exception(
                $"length(byte){totalLengthOfBytes} != length(count){responseBytes.Count}"
            );
            }
        ...
        return
            firstResult.Identifier == CommandIdentifier.ControlVariables.SIMSTEP && count.Value + 1 != results.Count
            ? throw new Exception()
            : results;
        }
    ...
    /// <returns></returns>
    private static (TraciResult traciResult, IEnumerable<byte> remainBytes) GetTraCIResult(
        this IEnumerable<byte> bytes
    )
        {
        var isLongResult = bytes.First() == 0;
        var resultLength = isLongResult
            ? ToInt32(bytes.Skip(1).Take(4).Reverse().ToArray())
            : bytes.First();
    ...
    private static TraciSubscriptionResponse ToSimStepResponse(this TraciResult traciResult)
        ...
        switch (commandType)
            ...
            case 0x0e: // 0xeX => VariableType Subscription Content
                {
                var (response, leftBytes) = TraciVariableSubscriptionResponse.FromBytes(
                    traciResult.Content
                );
                response.Identifier = traciResult.Identifier;
                return leftBytes.Any()
            ...
            case 0x09: // 0x9X => Object Context Subscription Content
                {
                var (response, leftBytes) = TraciContextSubscriptionResponse.FromBytes(
                    traciResult.Content
                );
                response.Identifier = traciResult.Identifier;
                return leftBytes.Any()
